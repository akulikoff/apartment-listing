  ...
  async (event): Promise<ApartmentListResponse> => {
    try {
      ...
      const filters: Partial<FilterParams> = {
        ...
        rooms: query.rooms ? query.rooms.split(',').map(r => parseInt(r)) : [],
      }
      ...
      return {
        ...
        meta: {
          total,
          filters: {
            priceRange: data.meta.filters.priceRange as [number, number],
            areaRange: data.meta.filters.areaRange as [number, number],
            roomsAvailable: data.meta.filters.roomsAvailable,
          },
        },
      }
    }
